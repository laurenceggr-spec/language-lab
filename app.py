...
if "config" not in st.session_state:
    st.session_state.config = {
        ...
        "consigne_eleve": st.query_params.get("c", "Présente-toi."),
        "identite_ia": "Alex, un ami curieux et bienveillant",
        "role_ia": "Tu es un tuteur de langue FWB."
    }
...
# --- 4. DASHBOARD PROFESSEUR ---
    ...
    with t_reg:
        ...
        st.session_state.config["mode"] = st.selectbox("Type d'activité :", ["Interaction (Dialogue)", "Production continue", "Tutorat"])
        st.session_state.config["grammaire"] = st.text_input("Attendus grammaticaux :", value=st.session_state.config["grammaire"])
    
    with t_cons:
        st.session_state.config["identite_ia"] = st.text_input("Identité de l'IA (ex: Alex, un ami) :", value=st.session_state.config["identite_ia"])
        st.session_state.config["role_ia"] = st.text_area("Consigne cachée (Prompt) :", value=st.session_state.config["role_ia"])
        st.session_state.config["consigne_eleve"] = st.text_area("Consigne pour l'élève :", value=st.session_state.config["consigne_eleve"])
...
# --- 5. INTERFACE ÉLÈVE ---
    ...
    audio_file = st.audio_input("Parle ici...")
    ...
    if audio_file:
        ...
        if st.session_state.last_processed_id != audio_id:
            with st.spinner("L'IA réfléchit..."):
                ...
                sys_p = f"Tu es {st.session_state.config['identite_ia']}. {st.session_state.config['role_ia']}. Scénario: {st.session_state.config['consigne_eleve']}. Langue: {st.session_state.config['langue']} (Niveau {st.session_state.config['niveau']})."
    ...
    with st.sidebar:
        if st.button("📊 GÉNÉRER MON BILAN FINAL"):
            with st.spinner("Analyse..."):
                p_bilan = f"Tu es un expert FWB. Analyse la conversation pour le niveau {st.session_state.config['niveau']} sur: AISANCE, RICHESSE ({st.session_state.config['grammaire']}), INTELLIGIBILITÉ. Réponds en FRANCAIS."
                ...
                st.session_state.messages.append({"role": "assistant", "content": f"🎯 **TON BILAN FWB :**\n\n{bilan_resp.choices[0].message.content}"})
                st.session_state.bilan_txt = bilan_resp.choices[0].message.content
                st.rerun()
        
        if "bilan_txt" in st.session_state:
            st.download_button("📥 Télécharger mon bilan", data=st.session_state.bilan_txt, file_name=f"bilan_{st.session_state.nom_eleve}.txt")

        if st.button("⬅️ Retour", key="ret"):
